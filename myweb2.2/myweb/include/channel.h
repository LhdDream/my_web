...
public:
    ...
    explicit channel(int sock_)
            : Socket_(std::make_shared<Socket>(sock_)),
   ...
   void handleRead();
    ...
    Epoll_event  get_event(){
        return Epoll_event{Socket_->fd(),type_};
    };


private:
    std::shared_ptr<Socket> Socket_; //对于每一个用户的fd进行保存
...
class channel_set {
public:
    explicit channel_set(poll &epoll) : epoll_(std::move(epoll)),respon_(std::make_unique<http_response>()),
